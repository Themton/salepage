...
import { getPages, createPage, deletePage, getAllOrders, updateOrderStatus } from '../lib/supabase';
  ...
  const exportCSV = () => {
    ...
    const rows = filteredOrders.map(o => {
      ...
      return [
        ...
        o.total || 0,
        o.total || 0,
        `${o.sp_pages?.name || ''} ${o.package_name || ''} ฿${(o.total||0).toLocaleString()}`,
      ];
    });
    ...
    <div style={{ minHeight: '100vh', background: bg, fontFamily: "'Noto Sans Thai', sans-serif" }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: 16 }}>
        ...
        {tab === 'pages' && (<div>
          ...
          {showNew && (
            <div style={{ background: '#fff', borderRadius: 14, padding: 20, marginBottom: 14, border: '1px solid #ddd', boxShadow: '0 4px 16px rgba(0,0,0,.06)' }}>
              ...
              <div key={pg.id} style={{ background: '#fff', borderRadius: 14, padding: 18, marginBottom: 10, border: '1px solid #eee', boxShadow: '0 1px 4px rgba(0,0,0,.03)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    ...
                    <button onClick={() => nav(`/admin/${pg.id}`)} style={{ background: blue, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>✏️ จัดการ</button>
                    <button onClick={() => handleDelete(pg.id, pg.name)} style={{ background: '#ffebee', color: red, border: 'none', borderRadius: 8, padding: '8px 10px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>🗑</button>
                  </div>
                </div>
              </div>
            );
